...
class CausalEventChainTracker:
    ...
    def add_events(self, events: List[Dict]):
        ...
        chain_length = len(events) #quanti eventi sono successi in quello step
    ...
    def get_statistics(self) -> Dict:
        ...
        total_events = sum(length * count for length, count in self.chain_stats.items()) #quantità totale di cambiamenti
        total_chains = sum(self.chain_stats.values()) # numero totale di step in cui è successo qualcosa di “significativo” - numero di step in cui è successo ≥1 evento
        avg_events_per_step = total_events / max(self.total_steps, 1)
...
class GenericSymbolicEnv(gym.Env):
    ...
    def step(self, action: int):
        ...
        # REWARD: catena eventi
        # events_this_step = len(detected_events)
...
def train_generic_dqn(env_factory: Callable, total_episodes=10000, max_steps=3000):
    ...
    for ep in range(total_episodes):
        ...
        steps = 0
        #Avg predicted action-value / epoch -> dice quanto la rete “crede” nelle sue azioni.
        q_values_epoch = []
        chosen_q_epoch = []
        ...
        while not done and steps < max_steps:
            ...
            q_values_epoch.append(q_vals.mean().item())
            ...
            if random.random() < epsilon: # 🎯 EXPLORATION - ogni quanto ignoro la rete e scelgo un'azione a caso
                ...
                chosen_q_epoch.append(q_vals[0, action].item())
        ...
        avg_q = np.mean(q_values_epoch) if q_values_epoch else 0.0
        avg_chosen_q = np.mean(chosen_q_epoch) if chosen_q_epoch else 0.0

        
        # Logging più dettagliato
        if ep % 50 == 0:
            avg_reward = np.mean(rewards_history[-50:]) if rewards_history else 0.0 #avg_score - finestra 50: reattivo
            ...
            print(
                    f"[Ep {ep:5d}] ε={epsilon:.3f} | "
                    f"    R: {total_reward:6.1f} (avg {avg_reward:6.1f}) | "
                    f"Survived: {survival_time:4.1f}s (avg_score {avg_survival:4.1f}s) | "
        #   f"Chains: {stats['total_chains']:3d} (len {avg_chain_len:.1f}) |"
                    f"Ev/step: {stats['avg_events_per_step']:.2f} | "
                    f"MaxEv: {stats['max_chain']:2d} | "
                    f"Q(avg): {avg_q:6.2f} | Q(ch): {avg_chosen_q:6.2f}"
                  )

        if ep >0 and ep % 200 == 0:
            avg_reward = np.mean(rewards_history[-200:]) if rewards_history else 0.0
            # print(f"Avg score (200 ep): {avg_reward:.2f}") #avg_score - finestra 200: trend vero
    ...
    # Salva modello
    model_path = os.path.join(SAVE_DIR, "generic_4_metrix.pth")
