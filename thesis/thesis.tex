...
\begin{itemize}
    ...
    Frames are captured from the environment, each represents the full observable state of the environment at a given time step.

    \item \textbf{Image preprocessing} \\
    Input images are resized to a fixed spatial resolution and normalized using standard ImageNet statistics. 
...
\begin{itemize}
    ...
    \item U-Net convolutional architecture for pixel-wise semantic segmentation.
    ...
    \item Deep Q-Learning (DQN) for policy learning.
...
The reinforcement learning agent is implemented using a fully generic symbolic environment. The environment automatically extracts all observable numerical and boolean attributes from the simulation object, forming a state representation without manual feature engineering.
Events are detected by comparing consecutive states and identifying significant changes in attribute values. These events are treated uniformly, without predefined semantic labels.
...
The agent is trained using Deep Q-Learning with experience replay, target networks, and $\varepsilon$-greedy exploration.


\paragraph{Reward Design and Intrinsic Motivation}
...
Event-based reward
Proportional to the number of detected events and the length of their causal chains.

Causal impact bonus
Measures how much the outcome of an action differs from a no-op baseline, encouraging interventions that meaningfully affect the environment.

Curiosity bonus
Derived from the prediction error of a learned forward dynamics model, which estimates the next state given the current state and action.

Event density bonus
Optionally rewards large overall changes in the state vector, promoting exploration of diverse configurations.
...
Several implementation choices improve computational efficiency and training stability:
\begin{itemize}
    \item Early stopping during segmentation training
    \item Gradient clipping in neural networks
    \item Limited observation dimensionality for reinforcement learning
    \item Adaptive target network update frequency

Serialization of models, logs, and symbolic populations
\end{itemize}



These optimizations ensure that the framework remains tractable despite its multi-layered architecture.
