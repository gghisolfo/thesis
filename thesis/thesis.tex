...
\subsubsection{Configuration 2: No Shaping - Density Enabled}
...
\begin{table}[h]
\centering
\caption{Performance comparison across model configurations. The table summarizes the results across configurations.}
\label{tab:results}
\begin{tabular}{lccc}
\toprule
...
no\_shaping\_no\_density -- last    & 99\%  & 23.98 & 1,687 \\
no\_shaping\_yes\_density -- best\_time & 100\% & 24.00 & 1,804 \\
...
yes\_shaping\_no\_density -- best\_reward & 0\%  & 0.00  & 23.54  \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{Final consideration}
Two configurations produced models that match or approach the supervised reference: pure causal reward (\textit{no\_shaping\_no\_density}) and causal reward with density (\textit{no\_shaping\_yes\_density}). The critical distinction between them lies not in peak performance but in training stability. The pure causal configuration converges reliably and maintains high performance across all checkpoints, while the density-augmented configuration reaches its optimum only transiently during training before destabilizing \cite{chiave69}.
The shaping experiments confirm that intrinsic structural signals and handcrafted bonuses are not straightforwardly additive. When the causal reward already provides a rich learning signal, introducing an additional shaped term risks disrupting the gradient landscape rather than supplementing it. This aligns with findings in the broader reward shaping literature, where imprecisely specified bonuses can lead to degenerate policies that exploit the shaping signal rather than solving the underlying task.
Taken together, these experiments demonstrate that causal interaction structure, as captured by the proposed intrinsic reward formulation, is both necessary and sufficient for learning to play Arkanoid without any external reward signal. The result is particularly notable given the task's requirement for sustained sequential behavior: the agent must not only contact the ball but maintain a long chain of interactions across dozens of bricks, coordinating its behavior over time horizons far beyond what a simple local reward signal would support.
