...
The decoder mirrors the encoder structure and employs transposed convolutions for upsampling. At each decoding stage, the upsampled feature maps are concatenated with the corresponding encoder outputs through skip connections. This mechanism preserves spatial context and improves boundary localization in the segmentation masks. To address potential spatial mismatches, bilinear interpolation is applied to ensure consistent feature map dimensions before concatenation.
...
Patch-to-object assignments are categorized as perfect, possible or no-direct, allowing the system to represent both expected and unexpected changes. Due to the sequential nature of frame processing, ambiguities frequently arise. To manage this uncertainty, the system maintains multiple competing interpretations in parallel, each corresponding to a distinct configuration of objects and inferred behaviors.
...
In addition, procedure provides two main advantages:
\begin{itemize}
...
\end{itemize}    
Although structured logs successfully encode both object identity and causal interactions, forming a solid foundation for the later reinforcement learning experiments based on causal intrinsic reward. However, naive log reconstruction from raw masks failed to produce meaningful causal chains, underscoring the necessity of refinement and heuristics. Therefore per-frame semantic segmentation alone is insufficient for reliable symbolic grounding due to noise and module limitations. A refinement pipeline that reconstructs coherent interaction logs is necessary to enable robust rule induction. 
...
As previously described, the system does not receive high-level semantic descriptions but rather sequences of frames composed of anonymous patches characterized only by low-level attributes. An episode corresponds to the complete temporal sequence of frames generated during a single gameplay session, capturing the dynamic evolution of the environment over time. To ensure exposure to diverse interaction patterns, the initial position of the ball is randomized at the beginning of each episode.
